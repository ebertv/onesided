...
class DialoguePredictionEvaluator:
    def __init__(self, claude_api_key: str, openai_api_key: str, use_few_shot: bool = True, token_count_only: bool = False, disable_anti_hallucination: bool = False):
...
- Prefixes like 'assistant:' or similar"""
        ...
        try:
            ...
            {{
              "detail_extraction": {{
                ...
                "fp": <integer>,
                "fn": <integer>,
                "precision_fraction": <float 0-1>,
                "recall_fraction": <float 0-1>
              }},
              "reasoning_and_scores": {{
                "semantic_similarity_reasoning": "reasoning for semantic similarity",
                "semantic_similarity": <integer 1-5>,
    ...
    async def run_evaluation(self, dataset_name: str, num_samples: int = 5, selected_scenario: str = None, skip_evaluation: bool = False, test_split_file: str = None, test_split_file_txt: str = None):
        ...
        try:
            ...
            # Save predictions immediately (without aggregate metrics)
            logger.info(f"\n💾 Saving predictions to {output_file}...")
            with open(output_file, "w") as f:
                json.dump(results, f, indent=2)
            logger.info(f"✅ Predictions saved successfully!")
            
            # Calculate and add aggregate metrics separately (if evaluation was done)
            if not skip_evaluation:
                ...
                try:
                    ...
                    logger.error(f"📁 Predictions are still saved in {output_file}")
                    logger.info(f"💡 You can manually inspect the predictions file to debug the metrics calculation")
                    # Don't re-raise the error - predictions are already saved
            else:
            ...
            logger.info(f"\n🎉 Evaluation completed! Final results in {output_file}")
            return results
    ...
    async def rerun_evaluation_from_file(self, predictions_file: str):
        ...
        # Save updated results first (without aggregate metrics)
        output_file = f"rerun_evaluation_{Path(predictions_file).stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        try:
            print(f"💾 Saving re-evaluated predictions to: {output_file}")
            logger.info(f"Saving re-evaluated predictions to: {output_file}")
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(existing_data, f, indent=2, ensure_ascii=False)
            
            print(f"✅ Re-evaluated predictions saved successfully!")
            logger.info(f"Re-evaluated predictions saved successfully!")
            
            # Calculate and add aggregate metrics separately
            print(f"📊 Calculating aggregate metrics...")
            logger.info(f"Calculating aggregate metrics...")
            try:
                existing_data["aggregate_metrics"] = self.calculate_scenario_metrics(predictions)
                
                # Update the file with aggregate metrics
                print(f"💾 Updating file with aggregate metrics...")
                with open(output_file, 'w', encoding='utf-8') as f:
                    json.dump(existing_data, f, indent=2, ensure_ascii=False)
                
                # Log summary metrics
                self.log_summary_metrics(existing_data["aggregate_metrics"])
                print(f"✅ Aggregate metrics calculated and saved!")
                logger.info(f"Aggregate metrics calculated and saved!")
                
            except Exception as metrics_error:
                print(f"❌ Error calculating aggregate metrics: {metrics_error}")
                logger.error(f"Error calculating aggregate metrics: {metrics_error}")
                print(f"📁 Re-evaluated predictions are still saved in {output_file}")
                logger.info(f"Re-evaluated predictions are still saved in {output_file}")
            
            print(f"\n🎉 Evaluation rerun complete! Final results in: {output_file}")
            print(f"\n📊 API Usage Summary:")
